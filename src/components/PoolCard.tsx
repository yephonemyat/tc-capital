...
export default function PoolCard({ name, apr, capacity, filled, chain, onPress }: { name: string; apr: number; capacity: number; filled: number; chain: string; onPress?: () => void; }) {
    const pct = Math.round((filled / capacity) * 100);
    return (
        <Pressable onPress={onPress} style={{ backgroundColor: '#fff', borderRadius: radii.lg, padding: spacing.lg, marginBottom: 12, ...shadow.card }}>
